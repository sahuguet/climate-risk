...
--
-- Specific random gnerator based on HDR2 for permutation of the flood table.
-- 
CREATE MACRO permute_flood(counter, year) AS hdr2(counter, varId:=year);

-- 
-- We create the flood table:
-- - 1 row per trial
...
DROP table if exists table_floods;
CREATE TABLE table_floods AS
(
WITH _table_floods AS (
    SELECT
    trial,
    'flood' AS peril,
    LIST(STRUCT_PACK(
...
FROM (SELECT UNNEST(generate_series(1, 10000)) AS trial) WHERE trial NOT IN (SELECT trial from _table_floods))
ORDER BY trial ASC
);
...
--
CREATE MACRO shuffle_table_floods_for_year(year) AS TABLE
SELECT (row_number() OVER ()) AS trial, year AS year, peril, event_summary
FROM (SELECT peril, event_summary from table_floods ORDER BY permute_flood(trial, year));
...
--
DROP TABLE IF EXISTS table_floods_10Y;
CREATE TABLE table_floods_10Y AS
(
SELECT trial, 1 AS year, peril, event_summary FROM table_floods
UNION
SELECT trial, year, peril, event_summary FROM shuffle_table_floods_for_year(2)
UNION
SELECT trial, year, peril, event_summary FROM shuffle_table_floods_for_year(3)
UNION
SELECT trial, year, peril, event_summary FROM shuffle_table_floods_for_year(4)
UNION
SELECT trial, year, peril, event_summary FROM shuffle_table_floods_for_year(5)
UNION
SELECT trial, year, peril, event_summary FROM shuffle_table_floods_for_year(6)
UNION
SELECT trial, year, peril, event_summary FROM shuffle_table_floods_for_year(7)
UNION
SELECT trial, year, peril, event_summary FROM shuffle_table_floods_for_year(8)
UNION
SELECT trial, year, peril, event_summary FROM shuffle_table_floods_for_year(9)
UNION
SELECT trial, year, peril, event_summary FROM shuffle_table_floods_for_year(10)
...
--
COPY table_floods_10Y TO 'table_floods_10Y.csv';

--
-- We (re)load the table.
-- We need to tell DuckDB how to reconstruct the `event_summary` field.
--
SELECT * FROM read_csv('table_floods_10Y.csv', columns = { 'trial': 'BIGINT', 'year': 'BIGINT', 'peril': 'STRING', 'event_summary': 'struct(event_id bigint, metadata json, economic_loss double, insured_loss double, pa_loss double)[]'});
...
FROM (SELECT trial, year, peril, unnest(event_summary) AS event FROM table_floods_10Y)) TO 'floods_10Y.parquet';